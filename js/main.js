  ...
  try {
    ...
    if(r.ok){
      ...
      if(d.pv && d.pv.length > pv.length && !reviewMode){
        ...
        const srvPV = d.pv.map(m=>Array.isArray(m)?{b:m[0],c:m[1]}:m);
...
reviewWorker.onmessage=({data:msg})=>{
  ...
  if(msg.type==='done'){
    reviewThink=false;
    reviewScore=msg.score; reviewPV=msg.pv||[];
  }
};
...
function enterReview(){
  ...
  reviewStep=0; reviewMode=true; showPostGame=false;
...
function setReviewStep(s){
  ...
  reviewStep=newStep;
  reviewWorker.postMessage({type:'start',timeMs:0,  // reset
...
function _newGame(){
  thinking=false; reviewMode=false; showPostGame=false;
  game=new Game(); aiPlayer=null; showMenu=true;
  hintMove=null; showHint=false; score=0; pv=[];
  moveEvals=[]; movePVs=[]; srvPat='';
}
...
function render(){
  ...
  if(showMenu){
    ...
    if(showPostGame)
      ui.drawPostGame(doneGame,doneEvals,getStats(),mouse);
  }
...
const inR=(pos,r)=>pos&&r&&pos.x>=r.x&&pos.x<r.x+r.w&&pos.y>=r.y&&pos.y<r.y+r.h;

function handleUp(e){
  const pos=getPos(e); const L=ui.L;
  ...
  if(reviewMode){
    ...
    if(rv.moveItems) for(const {r,step} of rv.moveItems)
  ...
  if(showMenu){
    for(const {r,role} of menuRects)
      if(inR(pos,r)){ aiPlayer=role; showMenu=false; _startAI(aiPlayer!==null&&game.player===aiPlayer); return; }
    return;
  }
  ...
  if(pos.x>=L.bx&&pos.x<L.bx+L.bp&&pos.y>=L.by&&pos.y<L.by+L.bp){
